...
public class Game {
  ...
  public void saisieProposition() {
    for (int x = 0; x < 4; x++) {
      System.out.println(
        "Choisissez la couleur du jeton " +
        x +
        " (" +
        proposition.toStringSelect(x) +
        ")"
      );
      try {
        int tInt = Integer.parseInt(in.readLine());
        if (tInt < 6 && tInt > -1) this.proposition.rowSet(
            new Jeton_couleur(tInt),
            x
          ); else {
          System.out.println(
            "Number out of range. Please make a selection from the key."
          );
          x--;
        }
      } catch (IOException e) {
        System.out.println(
          "Input not an integer. Please make a selection from the key."
        );
        x--;
  ...
  public boolean evalProposition() {
    ...
    this.indicators_data.donnees[this.counter] =
      this.ligneGagnante.computeSoluce(proposition).stringTab();
    ...
    this.counter++;
    // (attribut de la classe Game Renvoyez true si la proposition est correcte (�gale au code), false sinon.
    return eval;
  }
  ...
  public void nonGraphicsGame() {
    ...
    while (this.counter != 10 && !gagner) {
      ...
      if (eval) {
        ...
        System.out.println(
          this.proposition.toString() + " : La proposition n'est pas correcte"
        );
        System.out.println(
          this.ligneGagnante.computeSoluce(proposition).toString() + " : Indice"
        );
